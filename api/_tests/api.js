...
describe('Parser of Items from MELI API', () => {
  ...
  const data = JSON.parse(
    fs.readFileSync('./api/_mockData/items.json', 'utf8')
  )
  ...
  test('Can map data from MELI API with Internal API', () => {
    ...
    expect(parsedResult.author).toEqual({ name: 'Jorge', lastName: 'Buitrago' })
...
describe('Parser of Item Details from MELI API', () => {
  ...
  const itemDetail = JSON.parse(
    fs.readFileSync('./api/_mockData/itemDetail.json', 'utf8')
  )
  const itemDescription = JSON.parse(
    fs.readFileSync('./api/_mockData/itemDescription.json', 'utf8')
  )
  const itemCategory = JSON.parse(
    fs.readFileSync('./api/_mockData/itemCategory.json', 'utf8')
  )
